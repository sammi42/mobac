...
public class GUI extends JFrame {
	...
	private void createMainFrame(){
		...
		Dimension	dContent 	= new Dimension(1024,768);
		...
		// ... if that is not possible, try to set the look and feel associated to the system
		catch (Exception e){
			try {
				...
				// TODO Auto-generated catch block
				System.out.println("The selection of look and feel was not possible, due to: " + e1.toString());
				e1.printStackTrace();
			}
		}
	}
	...
	public void createLeftPanel() {
		...
		leftPanel.setBounds(3,3,281,731);
		...
		JScrollPane scrollPane = new JScrollPane(profilesJList, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	public void createRightPanel() {
		...
		rightPanel.setBounds(285,3,731,731);
		...
		JScrollPane scrollPane = new JScrollPane(previewPanel, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
	...
	public void addListeners(){
		...
		latMinTextField.getDocument().addDocumentListener(new JTextFieldListener());
		latMaxTextField.getDocument().addDocumentListener(new JTextFieldListener());
		longMinTextField.getDocument().addDocumentListener(new JTextFieldListener());
		longMaxTextField.getDocument().addDocumentListener(new JTextFieldListener());
	...
	public void initiateProgram(){
		...
		profilesVector = PersistentProfiles.load(new File(System.getProperty("user.dir") + fileSeparator + "profiles.xml"));
	...
	public String validateInput(boolean isCreateAtlasValidate) {
		...
		if(longMinTextField.getText().length() < 1) {
			...
			try {
				Double temp = Double.parseDouble(longMinTextField.getText());
		...
		if(longMaxTextField.getText().length() < 1) {
			...
			try {
				Double temp = Double.parseDouble(longMaxTextField.getText());
		...
		if(latMaxTextField.getText().length() < 1) {
			...
			try {
				Double temp = Double.parseDouble(latMaxTextField.getText());
		...
		if(latMinTextField.getText().length() < 1) {
			...
			try {
				Double temp = Double.parseDouble(latMinTextField.getText());
	...
	public boolean validateLatLongMinMax() {
		...
		Double latMax = Double.parseDouble(latMaxTextField.getText());
		Double latMin = Double.parseDouble(latMinTextField.getText());
		Double longMax = Double.parseDouble(longMaxTextField.getText());
		Double longMin = Double.parseDouble(longMinTextField.getText());
	...
	public void calculateNrOfTilesToDownload () {
		...
		if (validateInput(false).length() == 0) {
			...
			for (int i = 0; i < nrOfLayers; i ++) {
				...
				Point topLeft = GoogleTileUtils.toTileXY(Double.parseDouble(latMaxTextField.getText()), Double.parseDouble(longMaxTextField.getText()), zoomLevels[i]);
				Point bottomRight = GoogleTileUtils.toTileXY(Double.parseDouble(latMinTextField.getText()), Double.parseDouble(longMinTextField.getText()), zoomLevels[i]);
	
				totalNrOfTiles = totalNrOfTiles + Utilities.calculateNrOfTiles(new TileXYMinMaxAndZoom(topLeft, bottomRight, zoomLevels[i]));
			}
		...
		public void focusLost(FocusEvent arg0) {
			...
			if (component.indexOf("tileSizeWidthTextField") > -1 ) {
				...
				if (input.length() > 0) {
					...
					if (result == -1) {
						if (Integer.parseInt(input) < 50 ){
							JOptionPane.showMessageDialog(null, "\"" + input + "\" is not valid input of Custom size (W) \n\nAccepted values are between 50 and 1792", "Error", JOptionPane.ERROR_MESSAGE);
						}	
					}
				}
			}
			if (component.indexOf("tileSizeHeightTextField") > -1 ) {	
				...
				if (input.length() > 0) {
					...
					if (result == -1) {
						if (Integer.parseInt(input) < 50 ){
							JOptionPane.showMessageDialog(null, "\"" + input + "\" is not valid input of Custom size (H) \n\nAccepted values are between 50 and 1792", "Error", JOptionPane.ERROR_MESSAGE);
						}	
					}
				}
			}
	...
	private boolean handleInputInRealTime() {
		...
		if ((((this.getFocusOwner()).getClass()).toString()).equals("class javax.swing.JTextField")) {
			...
			if(input.length() > 0) {
				...
				if(jtfobjAsString.indexOf("latMaxTextField") > -1 || jtfobjAsString.indexOf("latMinTextField") > -1 || jtfobjAsString.indexOf("longMaxTextField") > -1 || jtfobjAsString.indexOf("longMinTextField") > - 1) {
					...
					if (result.length() > 0) {
						JOptionPane.showMessageDialog(null, "\"" + result + "\" is not valid input \n\nOnly accepted input is \"0-9\" and \".\"", "Error", JOptionPane.ERROR_MESSAGE);
						return false;
					}
				}
				...
				if(jtfobjAsString.indexOf("tileSizeWidthTextField") > -1) {
					...
					if (result > -1) {
						JOptionPane.showMessageDialog(null, "\"" + (char)result + "\" is not valid input \n\nOnly accepted input is \"0-9\"", "Error", JOptionPane.ERROR_MESSAGE);
						return false;
					} else if (input.length() > 3) {
						 if (Integer.parseInt(input) > 1792 ){
							 JOptionPane.showMessageDialog(null, "\"" + input + "\" is not valid input \n\nOnly accepted input is values between 50 and 1792", "Error", JOptionPane.ERROR_MESSAGE);
							 return false;
						 }	 
					}	
				}
				...
				if(jtfobjAsString.indexOf("tileSizeHeightTextField") > -1) {
					...
					if (result > -1) {
						...
						return false;
						 ...
						 if (Integer.parseInt(input) > 1792 ){
							 JOptionPane.showMessageDialog(null, "\"" + input + "\" is not valid input \n\nOnly accepted input are values between 50 and 1792", "Error", JOptionPane.ERROR_MESSAGE);
							 return false;
						 }
				...
				if(jtfobjAsString.indexOf("atlasNameTextField") > -1 ) {
					...
					if (result > -1) {
						JOptionPane.showMessageDialog(null, "\"" + (char)result + "\" is not valid input", "Error", JOptionPane.ERROR_MESSAGE);
						return false;
					}
				}
			}
	...
	public boolean retryDownloadAtlasTile(int xValue, int yValue, int zoomValue, File destinationFolder, int serverSwitcher) {
		...
		for (int i = 0; i < 10; i++) {
			...
			try {
				GoogleTileDownLoad.getImage(xValue, yValue, zoomValue,  destinationFolder, serverSwitcher, true);
				retryOk = true;
			}
			catch (IOException e) {
				retryOk = false;
	...
	public boolean retryDownloadPreviewTile(int xValue, int yValue, int zoomValue, File destinationFolder, int serverSwitcher) {
	...
	private class JPopupMenuListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			double [] coordinates = Utilities.calculatePreviewCoordinates(ProcessValues.getMouseXCoordinat(), ProcessValues.getMouseYCoordinat());
			...
			if (actionCommand.equals("Top left corner")) {
				...
				longMinTextField.setText(Utilities.nrOfDecimals(selectedLongMin, 6));
				...
				longMaxTextField.setText(Utilities.nrOfDecimals(selectedLongMin, 6));
				jpm.setVisible(false);
			}
		}	
	}
	...
	private class ButtonListener implements ActionListener{
		public void actionPerformed(ActionEvent e){
					...
					if (maxIsBiggerThanMin) {
						...
						if (tileSizeWidthComboBox.isEnabled() == false) {
							...
							try {
								...
								JOptionPane.showMessageDialog(null, "Custom tile size width value is not a valid integer value", "Errors", JOptionPane.ERROR_MESSAGE);
							}
						}
						if (tileSizeHeightComboBox.isEnabled() == false) {
							...
							try {
								...
								JOptionPane.showMessageDialog(null, "Custom tile size height value is not a valid integer value", "Errors", JOptionPane.ERROR_MESSAGE);
							}
						}
							...
							try {
								...
								atlasThread = new Thread(){
									...
									public void run(){
										...
										File ozi = new File(workingDir + fileSeparator + "ozi" + fileSeparator + formattedDateString);
										...
										File atlas = new File(workingDir + fileSeparator + "atlases" + fileSeparator + formattedDateString);
										...
										File atlasTar = new File(workingDir + fileSeparator+ "atlasestared" + fileSeparator + formattedDateString);
										...
										File crtba = new File(atlas.getAbsolutePath() + fileSeparator + "cr.tba");
										...
										for (int layer = 0; layer < nrOfLayers; layer++) {
											...
											if (ProcessValues.getAbortAtlasDownload()) {
												...
												/***
												 * In this section of code below, tiles for Atlas is being downloaded and put
												 * into folder "ozi"
												 **/
												...
												Point topLeft = GoogleTileUtils.toTileXY(Double.parseDouble(latMaxTextField.getText()), Double.parseDouble(longMaxTextField.getText()), zoom);
												Point bottomRight = GoogleTileUtils.toTileXY(Double.parseDouble(latMinTextField.getText()), Double.parseDouble(longMinTextField.getText()), zoom);

												int apMax = Utilities.calculateNrOfTiles(new TileXYMinMaxAndZoom(topLeft, bottomRight, zoom));
														...
														for (int j = xMin; j <= xMax; j++) {
															...
															if (ProcessValues.getAbortAtlasDownload()) {
																...
																try {
																	...
																	GoogleTileDownLoad.getImage(j, i, zoom,  oziZoom, serverSwitcher, true);
																}
																catch (IOException e) {

																	boolean retryOK;

																	retryOK = retryDownloadAtlasTile(j, i, zoom,  oziZoom, serverSwitcher);

																	if (retryOK == false) {
																		JOptionPane.showMessageDialog(null, "Something is wrong with connection to download server. Please check connection to internet and try again", "Error", JOptionPane.ERROR_MESSAGE);
																		System.exit(1);
																	}
																}
																serverSwitcher++;
												...
												if ((oziZoom.list().length) != ((yMax - yMin + 1) * (xMax - xMin + 1))) {
													JOptionPane.showMessageDialog(null, "Something is wrong with download of atlas tiles. Actual amount of downoladed tiles is not the same as the supposed amount of tiles downoladed. It might be connection problems to internet or something else. Please try again.", "Error", JOptionPane.ERROR_MESSAGE);
													System.exit(1);
												}

												File atlasFolder = new File(atlas.getAbsolutePath() + fileSeparator + mapName + zoom);
												atlasFolder.mkdir();
												...
												if(tileSizeWidthComboBox.isEnabled()) {
													tileSizeWidth = Integer.parseInt(tileSizeWidthComboBox.getSelectedItem().toString());
												}
												else {
													tileSizeWidth = Integer.parseInt(tileSizeWidthTextField.getText());
												}
												if(tileSizeHeightComboBox.isEnabled()) {
													tileSizeHeight = Integer.parseInt(tileSizeHeightComboBox.getSelectedItem().toString());
												}
												else {
													tileSizeHeight = Integer.parseInt(tileSizeHeightTextField.getText());
												}
																								
												OziToAtlas ota = new OziToAtlas(oziZoom, atlasFolder, tileSizeWidth, tileSizeHeight, mapName, zoom);
										...
										if (ProcessValues.getAbortAtlasDownload()) {
											JOptionPane.showMessageDialog(null, "Atlas download aborted", "Information", JOptionPane.INFORMATION_MESSAGE);
											ap.setButtonText();
										}
										else {
											ap.setButtonText();

											Utilities.createCR_TAR(atlas, atlasTar, new File(workingDir + fileSeparator + "tarwrkdir"));
											AtlasProgress ap = AtlasProgress.getInstance();
											ap.updateTarPrograssBar();
											Utilities.createTarPackedLayers(atlas, atlasTar);
											ap.updateTarPrograssBar();
											
											JOptionPane.showMessageDialog(null, "Atlas download completed", "Information", JOptionPane.INFORMATION_MESSAGE);
										}
			...
			if(actionCommand.equals("Save as profile")) {
				...
				if (inputValue != null) {
					...
					if (profileNameExists) {
						JOptionPane.showMessageDialog(null, "Profile name already exists, choose a different name", "Error", JOptionPane.ERROR_MESSAGE);
					}
					else {
						...
						if(errorDescription.equals("")){
							...
							theProfile.setLongitudeMax(Double.parseDouble(longMaxTextField.getText()));
							theProfile.setLongitudeMin(Double.parseDouble(longMinTextField.getText()));
							...
							if (!tileSizeWidthComboBox.isEnabled()) {
								theProfile.setCustomTileSizeWidth(Integer.parseInt(tileSizeWidthTextField.getText()));
							}
							else {
							...
							if (!tileSizeHeightComboBox.isEnabled()) {
								theProfile.setCustomTileSizeHeight(Integer.parseInt(tileSizeHeightTextField.getText()));
							}
							else {
							...
							JOptionPane.showMessageDialog(null, errorDescription, "Errors", JOptionPane.ERROR_MESSAGE);
						}
					}
				}
			}

			if(actionCommand.equals("Delete profile")) {
			...
			if(actionCommand.equals("Preview")) {
				...
				if(errorText.length() > 0) {
					...
					if (maxIsBiggerThanMin) {
						...
						tXY =  GoogleTileUtils.getTileXYMinMax(latMax, longMax, latMin, longMin);
						...
						rect = GoogleTileUtils.getTileRect(tXY.getXMax(), tXY.getYMin(), tXY.getZoom());
						ProcessValues.setPreviewLatMaxCoord(rect.getMaxY());
						
//						System.out.println("rect.getMaxY() : " + rect.getMaxY());
//						System.out.println("rect.getMinY() : " + rect.getMinY());
						
						// Get the lat min coord
						rect = GoogleTileUtils.getTileRect(tXY.getXMax(), tXY.getYMax(), tXY.getZoom());
						...
						rect = GoogleTileUtils.getTileRect(tXY.getXMin(), tXY.getYMin(), tXY.getZoom());
...
//						System.out.println("ProcessValues.getPreviewLatMaxCoord : " + ProcessValues.getPreviewLatMaxCoord());
//						System.out.println("ProcessValues.getPreviewLatMinCoord : " + ProcessValues.getPreviewLatMinCoord());
//						System.out.println("ProcessValues.getPreviewLongMaxCoord : " + ProcessValues.getPreviewLongMaxCoord());
//						System.out.println("ProcessValues.getPreviewLongMinCoord : " + ProcessValues.getPreviewLongMinCoord());
						
						pb = new PreviewProgressBar(0,Utilities.calculateNrOfTiles(tXY));
						pb.setVisible(true);
						...
						downloadThread = new Thread(){
							...
							public void run(){
								...
								for (int i = tXY.getYMin(); i <=tXY.getYMax(); i++) {
									for (int j = tXY.getXMin(); j <= tXY.getXMax(); j++) {
										...
										try {
											...
											GoogleTileDownLoad.getImage(j, i, tXY.getZoom(),  preview, serverSwitcher, false);
										}
										catch (IOException e) {

											boolean retryOK;

											retryOK = retryDownloadPreviewTile(j, i, tXY.getZoom(),  preview, serverSwitcher);
								...
								if(files.length > 0) {
									...
									// Hitta min och maxvärden för x och y, för att kunna
									// rendera den totala bilden i rätt ordning
									for (int i = 0; i < files.length; i++) {

										if(!files[i].isDirectory()){

											int intYValue = Integer.parseInt(files[i].getName().substring(1, files[i].getName().indexOf("x")));
											int intXValue = Integer.parseInt(files[i].getName().substring(files[i].getName().indexOf("x") + 1, files[i].getName().length() - 4));
									...
									// För att få ett startvärde på första filens x och y värde
									yValue = fileName.substring(1, fileName.indexOf("x"));
									xValue = fileName.substring(fileName.indexOf("x") + 1, fileName.length() - 4);

									nrOfXValues = 0;

									// Iterera igenom alla filer för att kontrollera hur många rader det finns i den
									// hämtade "kartarrayen"
									for (int i = 0; i < files.length; i++) {
										if (!files[i].isDirectory()) {

											if (((files[i].getName()).substring(1, fileName.indexOf("x")).equals(yValue))) {
								...
								for (int i = minYValue; i <= maxYValue; i++) {
									...
									for (int j = minXValue; j <= maxXValue; j++) {
										...
										try {
											BufferedImage image = ImageIO.read(new File(System.getProperty("user.dir") + fileSeparator + "preview" + fileSeparator + "y" + i + "x" + j + ".png"));
											ImageIO.write(image, "jpg", new FileOutputStream( new File(System.getProperty("user.dir") + fileSeparator + "preview" + fileSeparator + "overlay" + fileSeparator + "y" + i + "x" + j + ".jpg")));
										}
										catch (IOException iox) {
											System.out.println(iox);
										}
										...
										// 2 Rows
										if (nrOfYValues == 2 && (nrOfXValues > 2 || nrOfXValues < 2)) {
											...
											if (i == minYValue && j == minXValue) {
												...
												Point theOffset = GoogleTileUtils.getPixelOffsetInTile(latMax, longMin, tXY.getZoom());

												try {

													BufferedImage image = ImageIO.read(new File(System.getProperty("user.dir") + fileSeparator + "preview" + fileSeparator + "overlay" + fileSeparator + "y" + i + "x" + j + ".jpg"));

													Graphics graphics = image.getGraphics();
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect((int)theOffset.getX(), (int)theOffset.getY(), 256 - (int)theOffset.getX(), 1);
													graphics.fillRect((int)theOffset.getX(), (int)theOffset.getY(), 1, 256 - (int)theOffset.getY());


													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect((int)theOffset.getX() + 1, (int)theOffset.getY() + 1 , 256 - (int)theOffset.getX() - 1, 256 - (int)theOffset.getY() - 1);
													graphics.dispose();

													ImageIO.write(image, "png", new FileOutputStream( new File(System.getProperty("user.dir") + fileSeparator + "preview" + fileSeparator + "overlay" + fileSeparator + "y" + i + "x" + j + ".png")));
												}
												catch (IOException iox) {
													System.out.println(iox);
												}
											...
											if (i == minYValue && !(j == minXValue || j == maxXValue)) {
												...
												try {
													...
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect(0, (int)theOffset.getY(), 256, 1);

													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect(0, (int)theOffset.getY() + 1, 256, 256 - (int)theOffset.getY() - 1);
													graphics.dispose();
											...
											if (i == minYValue && j == maxXValue) {
												...
												Point theOffset = GoogleTileUtils.getPixelOffsetInTile(latMax, longMax, tXY.getZoom());
												...
												try {
													...
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect(0, (int)theOffset.getY() , 256 - (int)theOffset.getX(), 1);
													graphics.fillRect(256 - (int)theOffset.getX() - 1, (int)theOffset.getY(), 1, 256 - (int)theOffset.getY());

													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect(0, (int)theOffset.getY() + 1 , 256 - (int)theOffset.getX() - 1, 256 - (int)theOffset.getY() - 1);
													graphics.dispose();
											...
											if (i == maxYValue && j == minXValue) {
												...
												Point theOffset = GoogleTileUtils.getPixelOffsetInTile(latMin, longMin, tXY.getZoom());
												...
												try {
													...
													Graphics graphics = image.getGraphics();
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect((int)theOffset.getX(), (int)theOffset.getY() - 1, 256 - (int)theOffset.getX(), 1);
													graphics.fillRect((int)theOffset.getX(), 0, 1, (int)theOffset.getY());

													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect((int)theOffset.getX() + 1, 0, 256 - (int)theOffset.getX() - 1, (int)theOffset.getY() - 1);
													graphics.dispose();
											...
											if (i == maxYValue && !(j == minXValue || j == maxXValue)) {
												...
												try {
													...
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect(0, (int)theOffset.getY() - 1, 256, 1);

													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect(0, 0, 256, (int)theOffset.getY() - 1);
													graphics.dispose();
											...
											if (i == maxYValue && j == maxXValue) {
												...
												Point theOffset = GoogleTileUtils.getPixelOffsetInTile(latMin, longMax, tXY.getZoom());
												...
												try {
													...
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect(0, (int)theOffset.getY() - 1 , 256 - (int)theOffset.getX(), 1);
													graphics.fillRect(256 - (int)theOffset.getX() - 1, 0, 1, (int)theOffset.getY());


													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect(0, 0, 256 - (int)theOffset.getX() - 1, (int)theOffset.getY() - 1);
													graphics.dispose();
    										...
    										JLabel temp = new JLabel(new ImageIcon(System.getProperty("user.dir") + fileSeparator + "preview" + fileSeparator + "overlay" + fileSeparator + "y" + i + "x" + j + ".png"));
										...
										// 2 Columns
										if (nrOfXValues == 2 && (nrOfYValues > 2 || nrOfYValues < 2)) {
											...
											if (!(i == minYValue || i == maxYValue) && j == minXValue) {
												...
												try {
													...
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect((int)theOffset.getX(), 0, 1, 256);


													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect((int)theOffset.getX() + 1, 0, 256 - (int)theOffset.getX() - 1, 256);
													graphics.dispose();
											...
											if (!(i == minYValue || i == maxYValue) && j == maxXValue) {
												...
												try {
													...
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect(256 - (int)theOffset.getX() - 1, 0, 1, 256);

													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect(0, 0, 256 - (int)theOffset.getX() - 1, 256);
													graphics.dispose();
										...
										if (nrOfYValues != 2 && nrOfXValues != 2) {
											...
											if (i == minYValue && j == maxXValue) {
												...
												try {
													...
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect(0, (int)theOffset.getY() , (int)theOffset.getX(), 1);
													graphics.fillRect((int)theOffset.getX() - 1, (int)theOffset.getY(), 1, 256 - (int)theOffset.getY());


													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect(0, (int)theOffset.getY() + 1 , (int)theOffset.getX() - 1, 256 - (int)theOffset.getY() - 1);
													graphics.dispose();
											...
											if (!(i == minYValue || i == maxYValue) && j == maxXValue) {
												...
												try {
													...
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect((int)theOffset.getX() - 1, 0, 1, 256);

													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect(0, 0, (int)theOffset.getX() - 1, 256);
													graphics.dispose();
											...
											// Inner tiles
											if (!(j == minXValue || j == maxXValue || i == minYValue || i == maxYValue)) {
											...
											if (i == maxYValue && j == maxXValue) {
												...
												try {
													...
													graphics.setColor(new Color(0, 0, 0, 128));
													graphics.fillRect(0, (int)theOffset.getY() - 1 , (int)theOffset.getX(), 1);
													graphics.fillRect((int)theOffset.getX() - 1, 0, 1, (int)theOffset.getY());


													graphics.setColor(new Color(0, 255, 0, 64));
													graphics.fillRect(0, 0, (int)theOffset.getX() - 1, (int)theOffset.getY() - 1);
													graphics.dispose();
	...
	private class JListListener implements ListSelectionListener
	{
		public void valueChanged(ListSelectionEvent e)
		{
			...
			if (selectedIndex != -1)
			{
				...
				longMinTextField.setText(Double.toString(temp.getLongitudeMin()));
				longMaxTextField.setText(Double.toString(temp.getLongitudeMax()));
				...
				if(temp.getCustomTileSizeHeight() == 0) {
					...
					tileSizeHeightTextField.setText(Integer.toString(temp.getCustomTileSizeHeight()));
				}
	...
	private class JFrameListener extends WindowAdapter {
		...
		Thread getGoogleDownloadStringThread;
		...
		public void windowOpened(WindowEvent e) {
			...
			getDefaultMapThread = new Thread() {
				...
				public void run() {
					...
					downloadThread = new Thread(){
						...
						public void run(){
							...
							tXY = GoogleTileUtils.getTileXYMinMax(85.0, 179.0, -85.0, -179.0);
							...
							for (int i = minYValue; i <= maxYValue; i++) {
								...
								for (int j = minXValue; j <= maxXValue; j++) {
									...
									JLabel temp = new JLabel(new ImageIcon(System.getProperty("user.dir") + fileSeparator + "preview" + fileSeparator + "y" + i + "x" + j + ".png"));
									previewPanel.add(temp);
									previewPanel.updateUI();
								}
							}
	...
	private class PreviewMouseListener implements MouseListener {
		...
		public void mouseClicked(MouseEvent me) {
			...
			if (me.getButton() == 3) {
				...
				jpm.setVisible(true);
				jpm.setLocation(me.getX() + previewPanel.getLocationOnScreen().x, me.getY() + previewPanel.getLocationOnScreen().y);
				...
				double borderTileLatMax = GoogleTileUtils.getTileBorderCordinate(ProcessValues.getPreviewLatMaxCoord(), ProcessValues.getPreviewLongMaxCoord(), ProcessValues.getPreviewZoomValue(), "N");
				double borderTileLongMax = GoogleTileUtils.getTileBorderCordinate(ProcessValues.getPreviewLatMaxCoord(), ProcessValues.getPreviewLongMaxCoord(), ProcessValues.getPreviewZoomValue(), "E");

				double borderTileLatMin = GoogleTileUtils.getTileBorderCordinate(ProcessValues.getPreviewLatMinCoord(), ProcessValues.getPreviewLongMinCoord(), ProcessValues.getPreviewZoomValue(), "S");
				double borderTileLongMin = GoogleTileUtils.getTileBorderCordinate(ProcessValues.getPreviewLatMinCoord(), ProcessValues.getPreviewLongMinCoord(), ProcessValues.getPreviewZoomValue(), "W");
				...
				ProcessValues.setPreviewXResolution(((((ProcessValues.getPreviewLongMaxTile() - ProcessValues.getPreviewLongMinTile() + 1)) * 256) / longDiff));
				ProcessValues.setPreviewYResolution(((((ProcessValues.getPreviewLatMaxTile() - ProcessValues.getPreviewLatMinTile() + 1)) * 256) / latDiff));
			}
		}
